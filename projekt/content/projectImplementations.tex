...
The Sensor network project we've made consist of a sink and a mote. The temperature mote has one single timer which is fired with some constant frequency. When it has received $n$ measurements it send the results to the sink. This means our temperature mote is only on when not measuring or sensing and therefore it's using duty cycling. 
